    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/85 backdrop-blur-xl select-none animate-fade-in">
      <div className="relative w-full max-w-2xl rounded-3xl border border-zinc-800 bg-[#070709] shadow-2xl overflow-hidden flex flex-col max-h-[92dvh]">
        ...
        <div className="p-6 space-y-5 text-xs sm:text-sm overflow-y-auto flex-1">
          ...
          {!isConnected ? (
            <div className="p-8 text-center space-y-4 rounded-3xl bg-zinc-900/30 border border-zinc-800 flex flex-col items-center justify-center my-3">
              <div className="w-14 h-14 rounded-2xl bg-amber-950/40 border border-amber-500/30 flex items-center justify-center text-amber-400">
                <AlertTriangle className="w-7 h-7" />
              </div>
              <div className="space-y-1.5 max-w-sm">
                <h3 className="text-base font-bold text-white">No Provider Found</h3>
                <p className="text-xs text-zinc-400 font-sans leading-relaxed">
                  Engine is disconnected. To benchmark throughput (tokens/sec), time to first token (TTFT), and inference performance, connect a model provider or API key first.
                </p>
              </div>
              {onOpenSettings && (
                <button
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-bold transition shadow-xl ${
              !isConnected
          ...
          >
            {!isConnected ? (
              <>
                ...
                <span>No Provider Found — Benchmark Disabled</span>
              </>
            ) : isRunning ? (
              <>
