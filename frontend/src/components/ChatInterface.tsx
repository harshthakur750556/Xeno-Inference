...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ onReplayIntro }) => {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#030305] text-white overflow-hidden select-none font-sans">
      <aside
        className={`${
          isSidebarOpen ? 'w-64 sm:w-72' : 'w-0'
        } transition-all duration-300 ease-in-out bg-[#07070d] border-r border-white/10 flex flex-col justify-between overflow-hidden relative z-30`}
      >
        <div className="p-4 space-y-4 flex flex-col h-full overflow-y-auto">
          <div className="flex items-center gap-2.5 px-2 py-1">
            <ButterflySvg size={32} glow={false} animated={true} />
            <div className="flex flex-col">
              <span className="font-roman text-sm font-bold tracking-wider text-white">
                XENO
              </span>
              <span className="font-calligraphy text-xs text-purple-300 -mt-1">
                Inference
              </span>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-purple-600/30 to-cyan-600/30 hover:from-purple-600/50 hover:to-cyan-600/50 border border-purple-500/30 text-xs font-semibold text-purple-200 transition cursor-pointer shadow-sm"
          >
            ...
            <div className="space-y-1">
              {STARTER_PROMPTS.map((starter, sIdx) => (
                <button
                  ...
                  className="w-full text-left p-2.5 rounded-lg bg-white/[0.02] hover:bg-white/[0.06] border border-white/5 transition group cursor-pointer"
                >
                  <div className="text-xs font-medium text-zinc-300 group-hover:text-purple-300 truncate">
                    {starter.title}
                  </div>
          ...
          <div className="mt-auto pt-4 border-t border-white/10 space-y-2">
            <div className="p-3 rounded-xl bg-black/50 border border-white/5 space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-bold ${isBackendOnline ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {isBackendOnline ? 'CONNECTED' : 'LOCAL SIM'}
                </span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-zinc-500">THROUGHPUT:</span>
                <span className="text-cyan-300">{telemetry.avgThroughput} tok/s</span>
              </div>
              <div className="flex items-center justify-between text-[11px]">
                <span className="text-zinc-500">KV-CACHE:</span>
                <span className="text-purple-300">{telemetry.vramUsedGb} GB</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="h-16 border-b border-white/10 bg-[#07070d]/90 backdrop-blur-xl px-4 sm:px-6 flex items-center justify-between z-20">
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-xs font-medium transition cursor-pointer"
              >
                <div className={`w-2.5 h-2.5 rounded-full bg-gradient-to-r ${selectedModel.color}`} />
              ...
              {isModelDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-80 rounded-2xl bg-[#0c0c18] border border-white/15 shadow-2xl p-2 z-50 animate-fade-in space-y-1">
                  {AVAILABLE_MODELS.map((model) => (
                    ...
                    >
                      <div className={`w-3 h-3 rounded-full mt-1 bg-gradient-to-r ${model.color}`} />
            ...
            <div className="hidden sm:flex items-center gap-2 px-2.5 py-1 rounded-full bg-white/[0.03] border border-white/10 text-[11px] font-mono">
              <span className={`w-2 h-2 rounded-full ${isBackendOnline ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
              <span className="text-zinc-400">
                {isBackendOnline ? 'Rust Axum: Online (3001)' : 'Client Sim Engine'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-xs font-medium text-zinc-300 hover:text-white transition cursor-pointer"
            ...
            <button
              ...
              className="p-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-zinc-400 hover:text-white transition cursor-pointer"
              title="Configure inference parameters"
            >
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 space-y-6">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto space-y-6 my-auto select-none">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/20 via-purple-600/25 to-pink-500/20 blur-3xl rounded-full" />
                <ButterflySvg size={260} glow={true} animated={true} />
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-baseline justify-center gap-3">
                  ...
                  <span className="font-calligraphy text-4xl sm:text-5xl text-purple-300">
                    Inference
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full pt-2">
                {STARTER_PROMPTS.map((item, idx) => (
                  <button
                    ...
                    className="p-3.5 rounded-2xl bg-white/[0.02] hover:bg-white/[0.06] border border-white/10 hover:border-purple-500/30 text-left transition group cursor-pointer space-y-1"
                  >
                    <div className="text-xs font-semibold text-zinc-200 group-hover:text-purple-300 flex items-center justify-between">
                      <span>{item.title}</span>
                      <Zap className="w-3.5 h-3.5 text-zinc-500 group-hover:text-cyan-400" />
                    </div>
          ...
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {!isUser && (
                  <div className="flex-shrink-0 w-8 h-8 rounded-xl bg-gradient-to-tr from-purple-600 to-cyan-500 p-0.5 shadow-lg shadow-purple-600/20">
                    <div className="w-full h-full bg-[#0c0c16] rounded-[10px] flex items-center justify-center">
                      <ButterflySvg size={20} glow={false} animated={false} />
                    </div>
                  </div>
                )}
                  ...
                  {msg.attachments && msg.attachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-1">
                      {msg.attachments.map((att, aIdx) => (
                        <div
                          key={aIdx}
                          className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white/10 border border-white/10 text-xs font-mono text-zinc-300"
                        >
                  ...
                  <div
                    className={`rounded-2xl p-4 sm:p-5 select-text shadow-xl ${
                      isUser
                        ...
                        : 'bg-[#0e0e18] border border-white/10 text-zinc-100 rounded-tl-none'
                    }`}
                  >
                    ...
                    {!isUser && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className={`hover:text-white transition cursor-pointer ${
                            speakingMessageId === msg.id ? 'text-cyan-400 animate-pulse' : ''
                          }`}
                          title="Read aloud"
                        >
                ...
                {isUser && (
                  <div className="flex-shrink-0 w-8 h-8 rounded-xl bg-white/10 border border-white/10 flex items-center justify-center text-zinc-300">
          ...
          {isStreaming && (
            <div className="flex gap-3 sm:gap-4 max-w-4xl mx-auto justify-start">
              <div className="flex-shrink-0 w-8 h-8 rounded-xl bg-gradient-to-tr from-purple-600 to-cyan-500 p-0.5 shadow-lg shadow-purple-600/20">
                <div className="w-full h-full bg-[#0c0c16] rounded-[10px] flex items-center justify-center">
                  <ButterflySvg size={20} glow={false} animated={true} />
                </div>
              </div>

              <div className="flex flex-col space-y-2 max-w-[85%] sm:max-w-[78%]">
                <div className="rounded-2xl p-4 sm:p-5 bg-[#0e0e18] border border-white/10 text-zinc-100 rounded-tl-none shadow-xl">
                ...
                <div className="flex items-center gap-2 text-[11px] font-mono text-cyan-400 px-1">
        ...
        <footer className="p-4 sm:p-6 bg-gradient-to-t from-[#030305] via-[#030305] to-transparent z-20">
          <div className="max-w-4xl mx-auto space-y-3">
            {attachments.length > 0 && (
              <div className="flex flex-wrap gap-2 p-2 rounded-xl bg-black/60 border border-white/10">
                {attachments.map((att, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 px-3 py-1 rounded-lg bg-white/10 text-xs font-mono text-zinc-200"
                  >
            ...
            <div className="relative rounded-2xl bg-[#0c0c16]/90 backdrop-blur-2xl border border-white/15 focus-within:border-purple-500/50 shadow-2xl transition duration-300">
              <textarea
                ref={textareaRef}
                value={input}
                ...
                <div>
                  {isStreaming ? (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-semibold transition cursor-pointer shadow-lg shadow-red-600/20"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 text-white transition cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed shadow-lg shadow-purple-600/25"
                    >
            ...
            <div className="flex items-center justify-between text-[11px] font-mono text-zinc-500 px-2">
              ...
              <span>Rust Engine SSE: Ready</span>
            </div>
          </div>
        </footer>
      </div>
