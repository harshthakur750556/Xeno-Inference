  ...
  return (
    <header className="h-14 border-b border-stone-200/90 dark:border-stone-800/90 bg-white/95 dark:bg-stone-900/95 backdrop-blur-2xl px-3 sm:px-6 flex items-center justify-between z-30 relative select-none transition-colors duration-200 shrink-0">
      ...
      <div className="hidden lg:flex items-center space-x-2 text-xs font-mono">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-stone-50 dark:bg-stone-800/60 border border-stone-200/80 dark:border-stone-700/80 text-stone-700 dark:text-stone-300 truncate max-w-[200px]" title={systemMetrics.gpuRenderer}>
          <Monitor className="w-3.5 h-3.5 text-purple-500" />
          <span className="truncate">{systemMetrics.gpuRenderer.split(" ")[0]}</span>
        </div>
      ...
      {/* Right Controls: Mobile Metrics Drawer Trigger & Theme Toggle */}
      ...
      {/* Mobile Telemetry Dropdown / Modal Sheet */}
      {isMobileMetricsOpen && (
        <div className="absolute top-14 left-0 right-0 p-4 bg-white/95 dark:bg-stone-900/95 backdrop-blur-2xl border-b border-stone-200 dark:border-stone-800 z-40 shadow-xl lg:hidden font-mono text-xs space-y-2.5">
          <div className="flex items-center justify-between pb-2 border-b border-stone-100 dark:border-stone-800">
            ...
            <button onClick={() => setIsMobileMetricsOpen(false)} className="p-1 text-stone-400 hover:text-stone-800 dark:hover:text-stone-200">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="p-2 rounded-xl bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800">
              <div className="text-stone-400 text-[9px] uppercase font-bold">CPU Threads</div>
              <div className="font-bold text-stone-800 dark:text-stone-200">{systemMetrics.cpuCores} Cores</div>
            </div>
            <div className="p-2 rounded-xl bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800">
            ...
            <div className="p-2 rounded-xl bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800">
              <div className="text-stone-400 text-[9px] uppercase font-bold">Resolution</div>
              <div className="font-bold text-stone-800 dark:text-stone-200">{systemMetrics.screenResolution}</div>
            </div>
            <div className="p-2 rounded-xl bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800">
              <div className="text-stone-400 text-[9px] uppercase font-bold">Security</div>
              <div className="font-bold text-emerald-600 dark:text-emerald-400">{isAirGapped ? "Air-Gap" : "Tor SOCKS5"}</div>
            </div>
          </div>
        </div>
      )}
